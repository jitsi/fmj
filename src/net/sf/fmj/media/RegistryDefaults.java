...
{
    // com.sun and com.ibm are added only for compatibility with the reference
    // implementation.
    ...
    private static final boolean ENABLE_GSTREAMER = false; // GStreamer plugin
                                                           // is still a bit
                                                           // flaky, we'll
                                                           // disable by
                                                           // default.

    public static boolean DISABLE_OGG = false; // if ogg is not needed, class
                                               // loading is faster
    ...
    public static final int getDefaultFlags()
    {
        if (defaultFlags == -1)
        {
            ...
            defaultFlags = jmfDefaults ? RegistryDefaults.JMF
    ...
    public static List<Object> plugInList(int flags)
    {
        ...
        if ((flags & JMF) != 0)
        {
            ...
            result.add(new PlugInInfo(
                    ...
                    new Format[] { new RGBFormat(null, -1, Format.intArray,
            ...
            result.add(new PlugInInfo("com.ibm.media.codec.audio.PCMToPCM",
                    ...
                    new Format[] { new AudioFormat("LINEAR", -1.0, -1, -1, -1,
                            -1, -1, -1.0, Format.byteArray), },
                    javax.media.PlugInManager.CODEC));
            // this one appears to be designed to convert to RTP-friendly audio
            // formats?
            ...
            result.add(new PlugInInfo(
                    ...
                    new Format[] { new AudioFormat("LINEAR", -1.0, 16, 1, 0, 1,
                            -1, -1.0, Format.byteArray), },
                    new Format[] { new AudioFormat("gsm", -1.0, -1, -1, -1, -1,
                            -1, -1.0, Format.byteArray), },
                    javax.media.PlugInManager.CODEC));
            result.add(new PlugInInfo(
            ...
            result.add(new PlugInInfo(
                    ...
                    javax.media.PlugInManager.CODEC));
            result.add(new PlugInInfo(
                    "com.sun.media.codec.audio.mpa.Packetizer", new Format[] {
        ...
        // mgodehardt: disabled this, is added later, we ask the class for input
        // and output formats
        /*
         * if ((flags & FMJ) != 0) { result.add(new
         * PlugInInfo("net.sf.fmj.media.codec.video.jpeg.Packetizer", new
         * Format[] { new JPEGFormat(), }, new Format[] { new
         * VideoFormat("jpeg/rtp", null, -1, Format.byteArray, -1.0f), },
         * PlugInManager.CODEC)); }
         */
        ...
        if ((flags & JMF) != 0)
        {
            ...
            result.add(new PlugInInfo(
                    ...
                    new Format[] {}, javax.media.PlugInManager.RENDERER));
            result.add(new PlugInInfo(
                    "com.sun.media.renderer.video.AWTRenderer", new Format[] {
                            new RGBFormat(null, -1, Format.intArray, -1.0f, 32,
            ...
            result.add(new PlugInInfo("com.sun.media.multiplexer.audio.GSMMux",
                    ...
                    javax.media.PlugInManager.MULTIPLEXER));
            result.add(new PlugInInfo(
                    "com.sun.media.multiplexer.audio.MPEGMux", new Format[] {},
            ...
            result.add(new PlugInInfo("com.sun.media.multiplexer.audio.WAVMux",
                    ...
                    javax.media.PlugInManager.MULTIPLEXER));
            result.add(new PlugInInfo(
                    "com.sun.media.multiplexer.audio.AIFFMux", new Format[] {},
        ...
        if ((flags & FMJ) != 0)
        {
            ...
            // TODO: the filter graph builder can't quite deal with this one
            // yet:
            ...
            if ((flags & FMJ) != 0)
            {
                        ...
                        || className.startsWith("com.t4l.jmf"))
                {
                    if (className.equals("net.sf.fmj.ffmpeg_java.FFMPEGParser")
                            || className
                                    .equals("net.sf.fmj.theora_java.NativeOggParser"))
                    { // don't remove
                    } else
                    {
                        remove = true;
    ...
    public static void unRegisterPlugins(int flags)
    {
        ...
        final int[] types = new int[] {
                ...
                javax.media.PlugInManager.CODEC,
                javax.media.PlugInManager.EFFECT,
                javax.media.PlugInManager.RENDERER,
                javax.media.PlugInManager.MULTIPLEXER };
