...
public class Encoder extends AbstractCodec
{
    ...
    protected Format[] outputFormats = new Format[] { new AudioFormat(
            AudioFormat.GSM, 8000, 8, 1, Format.NOT_SPECIFIED,
    ...
    public Encoder()
    {
        ...
        this.inputFormats = new Format[] { new AudioFormat(AudioFormat.LINEAR,
                8000, 16, 1, AudioFormat.BIG_ENDIAN, AudioFormat.SIGNED,
                Format.NOT_SPECIFIED, Format.NOT_SPECIFIED, Format.byteArray) };
    ...
    public Format[] getSupportedOutputFormats(Format input)
    {
        ...
        } else
        {
            ...
            final AudioFormat result = new AudioFormat(AudioFormat.GSM,
                    inputCast.getSampleRate(), 8, 1, inputCast.getEndian(),
                    AudioFormat.SIGNED, 264, inputCast.getFrameRate(),
                    Format.byteArray);
    ...
    public int process(Buffer inputBuffer, Buffer outputBuffer)
    {
        ...
        if (outputBufferData == null
                || outputBufferData.length < GSM_BYTES * innerDataLength
                        / PCM_BYTES)
        {
            outputBufferData = new byte[GSM_BYTES
                    * (innerDataLength / PCM_BYTES)];
        ...
        } else
        {
            ...
            GSMEncoderUtil.gsmEncode(bigEndian, (byte[]) innerBuffer.getData(),
                    innerBuffer.getOffset(), innerDataLength, outputBufferData);
            ...
            System.arraycopy(innerContent, (innerDataLength / PCM_BYTES)
                    * PCM_BYTES, temp, 0, temp.length);
