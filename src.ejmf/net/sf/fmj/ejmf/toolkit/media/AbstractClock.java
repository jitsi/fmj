...
public class AbstractClock implements Clock
{
     ...
     * relationship between the <tt>Clock's</tt> <i>media time</i> and its
     * <tt>TimeBase</tt>.
     * <p>
     * For example, a rate of 2.0 indicates that <i>media time</i> will pass
     * twice as fast as the <tt>TimeBase</tt> time once the <tt>Clock</tt>
     * starts. Similarly, a negative rate indicates that the <tt>Clock</tt> runs
     * in the opposite direction of its <tt>TimeBase</tt>. All <tt>Clocks</tt>
     * are guaranteed to support a rate of 1.0, the default rate.
     * <tt>Clocks</tt> are not required to support any other rate.
     * <p>
     */
    ...
    /**
     * Get the time-base time the media is scheduled to start (if the Clock is
     * stopped), or the time-base time at which the Clock started (if the Clock
     * is started).
     */
    ...
    public synchronized Time mapToTimeBase(Time t) throws ClockStoppedException
    {
        ...
        return new Time((long) (((mCurrent - mStart) / rate) + tbStart));
    }
    ...
    /**
     * Set the <i>media time</i> at which you want the <tt>Clock</tt> to stop.
     * The <tt>Clock</tt> will stop when its <i>media time</i> passes the
     * stop-time. To clear the stop time, set it to: <tt>Clock.RESET</tt>.
     * <p>
     * You can always call <tt>setStopTime</tt> on a <i>Stopped</i>
     * <tt>Clock</tt>.
     * <p>
     * On a <i>Started</i> <tt>Clock</tt>, the stop-time can only be set
    ...
    public synchronized void stop()
    {
        if (isStarted)
        {
            mediaStartTime = calculateMediaTime(); // KAL: TODO: what is the
                                                   // reason for this?
    ...
    /**
     * syncStart the AbstractClock at the previously- specified time-base start
     * time.
     * <p>
