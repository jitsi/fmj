...
class ilbc_encoder
{
    ...
    public ilbc_encoder(int init_mode) throws Error
    {
        ...
        lpc_buffer = new float[ilbc_constants.LPC_LOOKBACK
            ...
            for (step_idx = 0, step = steps[step_idx]; step_idx < ilbc_constants.LSF_NUMBER_OF_STEPS;)
            {
                ...
                if (((hlp5 * (olds[old])) <= 0.0f) || (omega >= 0.5))
                {
                    if (step_idx == (ilbc_constants.LSF_NUMBER_OF_STEPS - 1))
                    {
                        ...
                        {
                            // System.out.println("acces index " + freq_idx +
                            // lsp_index);
    ...
    void AbsQuantW(float in[], /* (i) vector to encode */
            int in_idx, float syntDenum[], /*
                                            * (i) denominator of synthesis
                                            * filter
                                            */
            int syntDenum_idx, float weightDenum[], /*
                                                     * (i) denominator of
                                                     * weighting filter
                                                     */
            int weightDenum_idx, int out[], /* (o) vector of quantizer indexes */
            int len, /*
                      * (i) length of vector to encode and vector of quantizer
                      * indexes
                      */
            int state_first /*
    ...
    )
    {
        ...
        for (n = 0; n < len; n++)
        {
            ...
            if ((state_first != 0) && (n == ilbc_constants.SUBL))
            {
                ...
                /* synthesis and weighting filters on input */
                ilbc_common.AllPoleFilter(in, in_idx + n, weightDenum,
                        weightDenum_idx, len - n,
                        ilbc_constants.LPC_FILTERORDER);
            ...
            ilbc_common.AllPoleFilter(syntOutBuf, syntOut + n, weightDenum,
                    weightDenum_idx, 1, ilbc_constants.LPC_FILTERORDER);
    ...
    private void anaFilter(float In[], int in_idx, float a[], int a_idx,
            int len, float Out[], int out_idx, float mem[])
    {
        int i, j;
        int po, pi, pm, pa;
        ...
        System.arraycopy(In, in_idx + len - ilbc_constants.LPC_FILTERORDER,
    ...
    private void createAugmentedVec(int index, float buffer[], int buffer_idx,
            float cbVec[])
    {
        int ilow, j;
        int pp, ppo, ppi;
    ...
    private void filteredCBvecs(float cbvectors[], float mem[], int mem_idx,
            int lMem)
    {
        int i, j, k;
        int pp, pp1;
        ...
        tempbuff2 = new float[ilbc_constants.CB_MEML
                + ilbc_constants.CB_FILTERLEN];

        for (i = 0; i < ilbc_constants.CB_HALFFILTERLEN; i++)
            tempbuff2[i] = 0.0f;
        System.arraycopy(mem, mem_idx, tempbuff2,
                ilbc_constants.CB_HALFFILTERLEN - 1, lMem);
        for (i = lMem + ilbc_constants.CB_HALFFILTERLEN - 1; i < lMem
                + ilbc_constants.CB_FILTERLEN; i++)
            tempbuff2[i] = 0.0f;
        ...
        for (k = 0; k < lMem; k++)
        {
            ...
            {
                cbvectors[pos] += tempbuff2[pp]
                        * ilbc_constants.cbfiltersTbl[pp1];
                pp++;
                pp1--;
            }
            pos++;
    ...
    float residual[]) /* (i) lpc residual signal */
    {
        ...
        float[] ssqEn_win = { (float) 0.8, (float) 0.9, 1.0f, (float) 0.9,
                (float) 0.8 };

        float[] sampEn_win = { 1.0f / (float) 6.0, 2.0f / (float) 6.0,
                3.0f / (float) 6.0, 4.0f / (float) 6.0, 5.0f / (float) 6.0 };
        ...
        for (n = 1; n < this.ULP_inst.nsub - 1; n++)
        {
            ...
            {
                fssqEn[n] += (residual[pp]) * (residual[pp]);
                bssqEn[n] += sampEn_win[ilbc_constants.SUBL - l - 1]
                        * (residual[pp]) * (residual[pp]);
                pp++;
            }
        }
    ...
    public void iCBSearch(int index[], /* (o) Codebook indices */
            ...
            int intarget_idx, float mem[], /*
                                            * (i) Buffer for codebook
                                            * construction
                                            */
            ...
            int weightDenum_idx, float weightState[], /*
                                                       * (i) weighting filter
                                                       * state
                                                       */
            int block) /* (i) the sub-block number */
    {
        ...
        float[] buf = new float[ilbc_constants.CB_MEML + ilbc_constants.SUBL
                + 2 * ilbc_constants.LPC_FILTERORDER];
        ...
        System.arraycopy(mem, mem_idx, buf, ilbc_constants.LPC_FILTERORDER,
                lMem);
        ...
        ilbc_common.AllPoleFilter(buf, ilbc_constants.LPC_FILTERORDER,
                weightDenum, weightDenum_idx, lMem + lTarget,
                ilbc_constants.LPC_FILTERORDER);

        /* Construct the codebook and target needed */

        System.arraycopy(buf, ilbc_constants.LPC_FILTERORDER + lMem, target, 0,
                lTarget);
        ...
        for (stage = 0; stage < nStages; stage++)
        {
            ...
            if ((measure > max_measure)
                    && (Math.abs(ftmp) < ilbc_constants.CB_MAXGAIN))
            {
                best_index = 0;
                max_measure = measure;
                gain = ftmp;
            ...
            if (lTarget == ilbc_constants.SUBL)
            {
                ...
                searchAugmentedCB(20, 39, stage, base_size - lTarget / 2,
                        target, buf, ilbc_constants.LPC_FILTERORDER + lMem, a,
                        b, c, energy, invenergy);
                max_measure = a[0];
                best_index = b[0];
                gain = c[0];
            }
            ...
            else
            {
                ...
                if (lTarget == ilbc_constants.SUBL)
                {
                    ...
                    } else if (base_index < (base_size - 20))
                    {
                        ...
                        {
                            sInd -= (eInd - range);
                            // System.out.println("on retire " + eInd + " - " +
                            // range + " pour arriver a " + sInd);
                            eInd = range;
                        }
                    } else
            ...
            if (stage == 0)
            {
                ...
                for (j = 0; j < (range - 1); j++)
                {
                    energy[(ppe + 1)] = energy[ppe] + (cbvectors[ppi])
                            * (cbvectors[ppi]) - (cbvectors[ppo])
                            * (cbvectors[ppo]);
                    ppo--;
                    ppi--;
                    ppe++;
            ...
            if ((lTarget == ilbc_constants.SUBL) && (sIndAug != 0))
            {
                ...
                b[0] = best_index;
                c[0] = gain;
                searchAugmentedCB(sIndAug, eIndAug, stage, 2 * base_size - 20,
                        target, cbvectors, lMem, a, b, c, energy, invenergy);
                max_measure = a[0];
                best_index = b[0];
                gain = c[0];
            }
            ...
            if (stage == 0)
            {
                ...
                gain = ilbc_common.gainquant(gain, 1.0f, 32, gain_index,
                        gain_index_idx + stage);
            } else
            {
                if (stage == 1)
                {
                    gain = ilbc_common.gainquant(gain,
                            Math.abs(gains[stage - 1]), 16, gain_index,
                            gain_index_idx + stage);
                } else
                {
                    gain = ilbc_common.gainquant(gain,
            ...
            } else
            {
                if (index[index_idx + stage] < base_size)
                {
                    ...
                    {
                        createAugmentedVec(index[index_idx + stage] - base_size
                                + 40, buf, ilbc_constants.LPC_FILTERORDER
                                + lMem, aug_vec);
                        // pp=aug_vec;
                        pp = 0;
                        ppt = aug_vec;
                ...
                } else
                {
                    ...
                    {
                        pp = filterno * lMem - lTarget
                                - index[index_idx + stage] + filterno
                                * base_size;
                        // pp=cbvectors+filterno*lMem-lTarget-index[stage]+filterno*base_size;
                        ppt = cbvectors;
                    } else
                    {
                        createAugmentedVec(index[index_idx + stage]
                                - (filterno + 1) * base_size + 40, cbvectors,
                                filterno * lMem, aug_vec);
                        // pp=aug_vec;
                        pp = 0;
                        ppt = aug_vec;
                    }
        ...
        {
            ftmp = cene * ilbc_constants.gain_sq5Tbl[i]
                    * ilbc_constants.gain_sq5Tbl[i];
            ...
            float block[]) /* (o) speech vector to encode */
    {
        ...
        int[] gain_index = new int[ilbc_constants.CB_NSTAGES
                * ilbc_constants.NASUB_MAX];
        int[] extra_gain_index = new int[ilbc_constants.CB_NSTAGES];
        int[] cb_index = new int[ilbc_constants.CB_NSTAGES
                * ilbc_constants.NASUB_MAX];
        int[] extra_cb_index = new int[ilbc_constants.CB_NSTAGES];
        int[] lsf_i = new int[ilbc_constants.LSF_NSPLIT
        ...
        for (n = 0; n < this.ULP_inst.nsub; n++)
        {
            anaFilter(data, n * ilbc_constants.SUBL, syntdenum, n
                    * (ilbc_constants.LPC_FILTERORDER + 1),
                    ilbc_constants.SUBL, residual, n * ilbc_constants.SUBL,
                    this.anaMem);
        }
        ...
        StateSearchW(residual, start_pos, syntdenum, (start - 1)
                * (ilbc_constants.LPC_FILTERORDER + 1), weightdenum,
                (start - 1) * (ilbc_constants.LPC_FILTERORDER + 1), idxForMax,
                idxVec, this.ULP_inst.state_short_len, state_first);

        ilbc_common.StateConstructW(idxForMax[0], idxVec, syntdenum,
                (start - 1) * (ilbc_constants.LPC_FILTERORDER + 1),
        ...
        { /* put adaptive part in the end */
            ...
            /* setup memory */
            for (int li = 0; li < ilbc_constants.CB_MEML
                    - this.ULP_inst.state_short_len; li++)
                mem[li] = 0.0f;
            System.arraycopy(decresidual, start_pos, mem,
                    ilbc_constants.CB_MEML - this.ULP_inst.state_short_len,
            ...
            for (int li = 0; li < ilbc_constants.LPC_FILTERORDER; li++)
                weightState[li] = 0.0f;
            // memset(weightState, 0,
            // ilbc_constants.LPC_FILTERORDER*sizeof(float));

            /* encode sub-frames */

            iCBSearch(extra_cb_index, 0, extra_gain_index, 0, residual,
                    start_pos + this.ULP_inst.state_short_len, mem,
            ...
            ilbc_common.iCBConstruct(decresidual, start_pos
                    + this.ULP_inst.state_short_len, extra_cb_index, 0,
                    extra_gain_index, 0, mem, ilbc_constants.CB_MEML
                            - ilbc_constants.stMemLTbl,
                    ilbc_constants.stMemLTbl, diff, ilbc_constants.CB_NSTAGES);
        ...
        { /* put adaptive part in the beginning */
            ...
            for (k = 0; k < diff; k++)
            {
                reverseResidual[k] = residual[(start + 1) * ilbc_constants.SUBL
                        - 1 - (k + this.ULP_inst.state_short_len)];
            }
            ...
            iCBSearch(extra_cb_index, 0, extra_gain_index, 0, reverseResidual,
                    0, mem, ilbc_constants.CB_MEML - ilbc_constants.stMemLTbl,
                    ilbc_constants.stMemLTbl, diff, ilbc_constants.CB_NSTAGES,
                    weightdenum, (start - 1)
                            * (ilbc_constants.LPC_FILTERORDER + 1),
                    weightState, 0);
        ...
        if (Nfor > 0)
        {
            ...
            for (int li = 0; li < (ilbc_constants.CB_MEML - ilbc_constants.STATE_LEN); li++)
                mem[li] = 0.0f;
            // memset(mem, 0,
            // (ilbc_constants.CB_MEML-ilbc_constants.STATE_LEN)*sizeof(float));
            ...
            for (subframe = 0; subframe < Nfor; subframe++)
            {
                ...
                iCBSearch(cb_index, subcount * ilbc_constants.CB_NSTAGES,
                        ...
                        mem, ilbc_constants.CB_MEML
                                - ilbc_constants.memLfTbl[subcount],
                        ilbc_constants.memLfTbl[subcount], ilbc_constants.SUBL,
                        ilbc_constants.CB_NSTAGES, weightdenum,
                        (start + 1 + subframe)
                                * (ilbc_constants.LPC_FILTERORDER + 1),
                ...
                ilbc_common.iCBConstruct(decresidual, (start + 1 + subframe)
                        * ilbc_constants.SUBL, cb_index, subcount
                        * ilbc_constants.CB_NSTAGES, gain_index, subcount
                        * ilbc_constants.CB_NSTAGES, mem,
                        ilbc_constants.CB_MEML
                                - ilbc_constants.memLfTbl[subcount],
                        ilbc_constants.memLfTbl[subcount], ilbc_constants.SUBL,
                ...
                System.arraycopy(decresidual, (start + 1 + subframe)
                        * ilbc_constants.SUBL, mem, ilbc_constants.CB_MEML
                        - ilbc_constants.SUBL, ilbc_constants.SUBL);
        ...
        if (Nback > 0)
        {
            ...
            meml_gotten = ilbc_constants.SUBL
                    * (this.ULP_inst.nsub + 1 - start);
            ...
            for (k = 0; k < meml_gotten; k++)
            {
                mem[ilbc_constants.CB_MEML - 1 - k] = decresidual[(start - 1)
                        * ilbc_constants.SUBL + k];
            }
            for (int li = 0; li < (ilbc_constants.CB_MEML - k); li++)
                mem[li] = 0.0f;
            // memset(mem, 0, (ilbc_constants.CB_MEML-k)*sizeof(float));
            ...
            for (subframe = 0; subframe < Nback; subframe++)
            {
                ...
                iCBSearch(cb_index, subcount * ilbc_constants.CB_NSTAGES,
                        gain_index, subcount * ilbc_constants.CB_NSTAGES,
                        reverseResidual, subframe * ilbc_constants.SUBL, mem,
                        ilbc_constants.CB_MEML
                                - ilbc_constants.memLfTbl[subcount],
                        ilbc_constants.memLfTbl[subcount], ilbc_constants.SUBL,
                        ilbc_constants.CB_NSTAGES, weightdenum,
                ...
                // memcpy(mem, mem+ilbc_constants.SUBL,
                // (ilbc_constants.CB_MEML-ilbc_constants.SUBL)*sizeof(float));
                System.arraycopy(reverseDecresidual, subframe
                        * ilbc_constants.SUBL, mem, ilbc_constants.CB_MEML
                        - ilbc_constants.SUBL, ilbc_constants.SUBL);
                // memcpy(mem+ilbc_constants.CB_MEML-ilbc_constants.SUBL,
        ...
        for (ulp = 0; ulp < 3; ulp++)
        {
            ...
            for (i = 0; i < this.ULP_inst.nasub; i++)
            {
                for (k = 0; k < ilbc_constants.CB_NSTAGES; k++)
                {
                    pack = bytes.packsplit(cb_index[i
                            * ilbc_constants.CB_NSTAGES + k],
                            this.ULP_inst.cb_index[i][k][ulp],
                            this.ULP_inst.cb_index[i][k][ulp]
                                    + this.ULP_inst.cb_index[i][k][ulp + 1]
                                    + this.ULP_inst.cb_index[i][k][ulp + 2]);
                    firstpart = pack.get_firstpart();
                    cb_index[i * ilbc_constants.CB_NSTAGES + k] = pack
                            .get_rest();
            ...
            for (i = 0; i < this.ULP_inst.nasub; i++)
            {
                for (k = 0; k < ilbc_constants.CB_NSTAGES; k++)
                {
                    pack = bytes.packsplit(gain_index[i
                            * ilbc_constants.CB_NSTAGES + k],
                    ...
                    firstpart = pack.get_firstpart();
                    gain_index[i * ilbc_constants.CB_NSTAGES + k] = pack
                            .get_rest();
    ...
    public void levdurb(float a[], /*
                                    * (o) lpc coefficient vector starting with
                                    * 1.0f
                                    */
    ...
    void LPCencode(float syntdenum[], /*
                                       * (i/o) synthesis filter coefficients
            ...
            float data[]) /* (i) lsf coefficients to quantize */
    {
        ...
        change = ilbc_common.LSF_check(lsfdeq, ilbc_constants.LPC_FILTERORDER,
                this.ULP_inst.lpc_n);
        // System.out.println("check gives " + change);
        SimpleInterpolateLSF(syntdenum, weightdenum, lsf, lsfdeq, this.lsfold,
    ...
    void LSFinterpolate2a_enc(float a[], /* (o) lpc coefficients */
            ...
            int lsf2_idx, float coef, /*
                                       * (i) weighting coefficient to use
                                       * between lsf1 and lsf2
                                       */
    ...
    )
    {
        ...
        ilbc_common.interpolate(lsftmp, lsf1, lsf2, lsf2_idx, coef,
                ((int) length));
    ...
    private void searchAugmentedCB(int low, /* (i) Start index for the search */
            ...
            float buffer[], /*
                             ...
                             */
            int buffer_idx, float max_measure[], /*
                                                  * (i/o) Currently maximum
                                                  * measure
                                                  */
    ...
    )
    {
        ...
        for (icount = low; icount <= high; icount++)
        {
            ...
            if ((measure > max_measure[0])
                    && (Math.abs(ftmp) < ilbc_constants.CB_MAXGAIN))
            {
                // System.out.println("new best index at " + tmpIndex +
                // ", where icount = " + icount);
            ...
            float data[]) /* (i) new data vector */
    {
        ...
        is = ilbc_constants.LPC_LOOKBACK + ilbc_constants.BLOCKL_MAX
        ...
        for (k = 0; k < this.ULP_inst.lpc_n; k++)
        {
            ...
            {
                window(temp, ilbc_constants.lpc_asymwinTbl, this.lpc_buffer,
                        is, ilbc_constants.BLOCKL_MAX);
            }

            autocorr(r, temp, ilbc_constants.BLOCKL_MAX,
                    ilbc_constants.LPC_FILTERORDER);
            window(r, r, ilbc_constants.lpc_lagwinTbl, 0,
                    ilbc_constants.LPC_FILTERORDER + 1);

            levdurb(lp, temp, r, ilbc_constants.LPC_FILTERORDER);
            ilbc_common.bwexpand(lp2, 0, lp,
                    ilbc_constants.LPC_CHIRP_SYNTDENUM,
                    ilbc_constants.LPC_FILTERORDER + 1);
        ...
        System.arraycopy(this.lpc_buffer, ilbc_constants.LPC_LOOKBACK
                + ilbc_constants.BLOCKL_MAX - is, this.lpc_buffer, 0, is);
    ...
    void SimpleInterpolateLSF(float syntdenum[], /*
                                                  * (o) the synthesis filter
                                                  * denominator resulting from
                                                  * the quantized interpolated
                                                  * lsf
                                                  */
            float weightdenum[], /*
                                  * (o) the weighting filter denominator
                                  * resulting from the unquantized interpolated
                                  * lsf
                                  */
            float lsf[], /* (i) the unquantized lsf coefficients */
            float lsfdeq[], /* (i) the dequantized lsf coefficients */
            float lsfold[], /*
                             * (i) the unquantized lsf coefficients of the
                             * previous signal frame
                             */
            float lsfdeqold[], /*
                                * (i) the dequantized lsf coefficients of the
                                * previous signal frame
                                */
        ...
        } else
        {
            ...
            for (i = 0; i < this.ULP_inst.nsub; i++)
            {
                ...
                for (int li = 0; li < lp_length; li++)
                    // System.out.println("interpolate syntdenum [" + (li+pos)
                    // +"] is worth " + syntdenum[li+pos]);
                    // memcpy(syntdenum+pos,lp,lp_length*sizeof(float));
                    LSFinterpolate2a_enc(lp, lsfold, lsf, 0,
                            ilbc_constants.lsf_weightTbl_20ms[i], length);
                ilbc_common.bwexpand(weightdenum, pos, lp,
    ...
    void SimplelsfQ(float lsfdeq[], /*
            ...
            float lsf[], /*
                          * (i) the lsf coefficient vector to be quantized
                          * (dimension FILTERORDER )
                          */
    ...
    )
    {
        ...
        if (lpc_n == 2)
        {
            ...
            SplitVQ(lsfdeq, ilbc_constants.LPC_FILTERORDER, index,
                    ilbc_constants.LSF_NSPLIT, lsf,
                    ilbc_constants.LPC_FILTERORDER, ilbc_constants.lsfCbTbl,
                    ilbc_constants.LSF_NSPLIT, ilbc_constants.dim_lsfCbTbl,
                    ilbc_constants.size_lsfCbTbl);
        }
    }
            ...
            int cbsize[]) /* the number of vectors in the codebook */
    {
        ...
        for (i = 0; i < nsplit; i++)
        {
            vq(qX, X_pos + qX_idx, index, i + index_idx, CB, cb_pos, X, X_pos
                    + X_idx, cbsize[i], dim[i]);
            X_pos += dim[i];
                              ...
                              */
    {
        ...
        ilbc_common.ZeroPoleFilter(tmpbuf, tmp, numerator, syntDenum,
                syntDenum_idx, 2 * len, ilbc_constants.LPC_FILTERORDER,
                foutbuf, fout);
        for (k = 0; k < len; k++)
        {
        ...
        AbsQuantW(foutbuf, fout, syntDenum, syntDenum_idx, weightDenum,
                weightDenum_idx, idxVec, len, state_first);
    }
